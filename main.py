...
from app.data import CustomerNotLinkedError, data_store, statement_store
...
async def bootstrap(user: dict = Depends(get_current_user)) -> dict:
    try:
        identity_subject = user.get("sub")
        email = user.get("email")
        customer_summary = data_store.get_customer_summary(
            identity_subject=identity_subject,
            email=email,
        )
        ...
        if linked_accounts == 0:
            return {
                ...
                "user": user,
                "data_source": "oracle_sqlcl" if data_store.uses_oracle else "in_memory_demo",
...
async def get_recent_activity(user: dict = Depends(get_current_user)) -> dict:
    try:
        return {
            "recent_activity": data_store.recent_activity(
                identity_subject=user.get("sub"),
                email=user.get("email"),
            )
        }
